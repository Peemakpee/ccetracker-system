                        ...
                        <div class="card card-danger">
                            ...
                            <div class="card-body">
                                <div class="form-group">
                                    <label for="phcomment">Comment of Program Head:</label>
                                    <div id="phcomment" class="form-control" style="height: auto; overflow-y: auto;">{{
                                        document.comment }}</div>
                                </div>
                                <div class="form-group">
                                    <label for="comment">Comment:</label>
                                    <textarea id="comment" class="form-control" v-model="document.deanComment"></textarea>
                                </div>
                                <div class="form-group">
...
export default {
    ...
    computed: {
        ...
        googleViewLink() {
            const url = this.document.firebaseUrl_wSign || 'fallback_value';
    ...
    methods: {
        ...
        async uploadSignedFile() {
            ...
            this.folderPath = `tracking/phase2/${this.document.status.toLowerCase()}_dean/${this.document.program}/${this.document.academic_year}/user ${this.document.user_id}`;
            ...
            try {
                ...
                this.document.firebaseUrl_wSignDean = downloadURL;
        ...
        downloadFile() {
            ...
            if (this.document.firebaseUrl_wSign) {

                window.open(this.document.firebaseUrl_wSign, '_blank');
            } else {
        ...
        async saveChanges() {
            ...
            const comment = this.document.deanComment;
            ...
            if (status === 'Approve' || status === 'To Be Modified') {
                ...
                try {
                    ...
                    const statusUpdateEndpoint = 'http://127.0.0.1:8000/api/update-approve-status';
            ...
            if (status === 'Approve' || status === 'To Be Modified') {
                ...
                try {
                    ...
                    const statusUpdateEndpointReupload = 'http://127.0.0.1:8000/api/update-reuploadDean-status';
            ...
            if (status === 'Approve' || status === 'To Be Modified') {
                ...
                if (status === 'To Be Modified') {
                    formData.append('firebaseUrl_wSignDean', this.document.firebaseUrl_wSign);
                    formData.append('file_name', this.document.file_name);
                    formData.append('file_path', this.folderPath);
                } else {
                    formData.append('firebaseUrl_wSignDean', this.document.firebaseUrl_wSignDean);
                ...
                try {
                    let apiEndpoint = '';
                    if (status === 'Approve') {
                        apiEndpoint = 'http://127.0.0.1:8000/api/approved-dean';
                    } else if (status === 'To Be Modified') {
                        apiEndpoint = 'http://127.0.0.1:8000/api/tbm-dean';
                    }
            ...
            this.document.deanComment = '';

            this.$router.push({ name: 'admin.pendingDean' });
        },
    },
    mounted() {
        const documentId = this.$route.params.id;
        ...
        let apiUrl;
        if (source === 'phase2') {
            apiUrl = `http://127.0.0.1:8000/api/get-approvedPh/${documentId}`;
        } else if (source === 'reuploadDean') {
            apiUrl = `http://127.0.0.1:8000/api/get-reuploadedDean/${documentId}`;
        } else {
        ...
        axios.get(apiUrl)
            .then(response => {
                if (source === 'reuploadDean' && response.data.data) {
