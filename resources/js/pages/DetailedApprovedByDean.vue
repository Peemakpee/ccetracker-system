    ...
    <div class="content">
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0 text-dark">Detailed View of the Approved Deliverable</h1>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <div class="card card-primary card-outline card-custom-outline">
                            ...
                            <div class="card-body">
                                ...
                                <div class="card card-info">
                                    ...
                                    <iframe :src="googleViewLink" style="width:100%; height:536px;"
                        ...
                        <div class="card card-danger">
                            ...
                            <div class="card-body">
                                <div class="form-group">
                                    <label for="phcomment">Comment of the Dean:</label>
                                    <div id="phcomment" class="form-control" style="height: auto; overflow-y: auto;">{{
                                        document.comment }}</div>
                                </div>
                                <div class="form-group" v-if="document.deanStatus === 'Changes Required'">
                                    <label for="comment">Change Description:</label>
                                    <textarea id="comment" class="form-control"
                                        v-model="document.change_description"></textarea>
                                </div>
                                <div class="form-group">
                                    <label for="status">Status:</label>
                                    <select id="status" class="form-control" v-model="document.deanStatus">
...
export default {
    data() {
        return {
            document: {
                ...
                submittedBy: document.user_name,
                ...
                dateUploadByUser: document.created_at,
    ...
    computed: {
        ...
        googleViewLink() {
            const url = this.document.firebaseUrl_wSignDean || 'fallback_value';
    ...
    methods: {
        async updateShowStatus() {
            ...
            try {
                const response = await axios.post(`http://127.0.0.1:8000/api/update-approve-dean-show/${documentId}`, {
                });
                console.log('Show status updated:', response.data);
            } catch (error) {
                console.error('An error occurred while updating show status:', error);
            }
        },
        ...
        async uploadSignedFile() {
            ...
            const documentId = this.$route.params.id;
            this.folderPath = `tracking/files_with_changes/${this.document.deanStatus.toLowerCase()}/${this.document.program}/${this.document.academic_year}/user ${this.document.user_id}`;
            ...
            try {
                ...
                this.document.firebaseUrl_changes = downloadURL;
        ...
        downloadFile() {
            ...
            if (this.document.firebaseUrl_wSignDean) {

                window.open(this.document.firebaseUrl_wSignDean, '_blank');
            } else {
        ...
        async saveChanges() {
            ...
            const deanStatus = this.document.deanStatus;
            const change_description = this.document.change_description;
            const deliverableType = this.document.deliverable_type;
            const fileName = this.signedFile.name;
            ...
            formData.append('status', deanStatus);
            formData.append('change_description', change_description);
            formData.append('firebaseUrl_changes', this.document.firebaseUrl_changes);
            ...
            try {
                ...
                let apiEndpoint = '';
                if (deanStatus === 'Changes Required') {
                    apiEndpoint = 'http://127.0.0.1:8000/api/changes-required';
                }
            ...
            this.document.deanStatus = '';
            this.document.change_description = '';

            this.$router.push({ name: 'admin.approvedByDean', params: { program: this.document.program } });
        },
    },
    mounted() {
        const documentId = this.$route.params.id;
        ...
        axios
            .get(`http://127.0.0.1:8000/api/get-approvedDean/${documentId}`)
            .then(response => {
                console.log('API Response:', response.data);
                this.document = response.data;
                console.log('Firebase URL:', this.document.firebaseUrl_wSignDean);
            })
            .catch(error => {
