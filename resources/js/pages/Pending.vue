  ...
  <section class="content">
    <div class="card">
      <div class="card-header custom-docheader">
        <h3 class="card-title custom-title">Pending Deliverables for Approval of PH</h3>
            ...
            <tr v-for="upload in (searchQuery ? filteredCombinedUploads : paginatedCombinedUploads)" :key="upload.id">
              ...
              <td class="project-actions text-right">
                <div class="btn-group" role="group" aria-label="Actions">
                  <router-link :to="`/admin/detailedUpload/${upload.id}?source=${upload.source}`"
...
export default {
  ...
  computed: {
    ...
    combinedUploads() {
      const normalizedPhase1Uploads = this.phase1Uploads.map(upload => ({
        id: upload.id,
      ...
      const normalizedReuploads = this.reuploads.map(upload => ({
      ...
      return [...normalizedPhase1Uploads, ...normalizedReuploads];
    },
  ...
  methods: {
    ...
    archiveUpload(id, source) {
      ...
      axios
        .post(`http://127.0.0.1:8000/api/archive-upload/${id}`, { source })
        .then(() => {
          if (source === 'phase1') {
            this.phase1Uploads = this.phase1Uploads.filter(upload => upload.id !== id);
          } else if (source === 'reupload') {
            this.reuploads = this.reuploads.filter(upload => upload.document_id !== id);
          }
        })
        .catch(error => {
  ...
  mounted() {
    ...
    let apiUrl = 'http://127.0.0.1:8000/api/get-upload';
    ...
    axios.get(apiUrl)
      .then(response => {
        if (Array.isArray(response.data)) {
          this.phase1Uploads = response.data;
        } else {
    ...
    let apiUrlReuploads = 'http://127.0.0.1:8000/api/get-reuploads';

    if (!isDean && !isAA) {
      apiUrlReuploads += `/${program}`; 
    }

    axios.get(apiUrlReuploads)
      .then(response => {
        if (Array.isArray(response.data)) {
          this.reuploads = response.data;
          console.log('Reuploads:', this.reuploads);
        } else {
          console.error('Unexpected data format:', response.data);
        }
      })
